...
DATA_FOLDER <- "C:/Research_stuff/bark_contour/data"
# assume working directory is location of script in
# ./scripts
RESULTS_FOLDER <- "C:/Research_stuff/bark_contour/results"
...
### function to look up tree diameter, etc.
...
makeContour <- function(id, method=CURVE_METHOD) { 
  ...
  pfile = file.path(DATA_FOLDER, "coord_data", paste(id, "pts.txt", sep=""))
  cfile = file.path(DATA_FOLDER, "coord_data", paste(id, "xy.txt", sep=""))
...
length(unique(results.sum$TreeID))
# and make example figure

makeContourPlot(makeContour("QUST021"))
